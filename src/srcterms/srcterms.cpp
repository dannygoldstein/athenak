...
//! \brief Simple isotropization of parallel and perpendicular pressures.

void SourceTerms::CGLCollisionalRelax(const DvceArray5D<Real> &w0, const Real bdt,
                                      DvceArray5D<Real> &u0) {
  ...
  KOKKOS_LAMBDA(int m, int k, int j, int i) {
    ...
    Real p_iso = (ppar + 2.0*pperp)/3.0;
    Real dppar = (p_iso - ppar)*nu*bdt;
    Real dpperp = (p_iso - pperp)*nu*bdt;
    u0(m,ippar,k,j,i) += dppar;
    u0(m,ipperp,k,j,i) += dpperp;
  });
